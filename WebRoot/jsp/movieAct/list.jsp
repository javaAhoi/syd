				...
				<div class="container-fluid">
					<div class="row">
						...
						<div class="middle_top_nav">
							<a href="${ctx}/index">电影</a><a href="javascript:;" class="active">演员列表</a>
						</div>
						<div class="middle_main paddingLR10 marginB26">
							<div class="col-lg-12 col-md-12">
						    	<div id="custom-toolbar1">
							    	<form action="${ctx}/movieAct/list" method="post">
							    		<input type="text" class="form-control" name="search_movieAct_name" value="${search_movieAct_name}" placeholder="输入演员名搜索" />
								    	<input type="submit" value="检索" style="width: auto;" class="btn btn-success marginT10" />
								    	<a href="${ctx}/movieAct/input?opera=create" style="width: auto;" class="btn btn-success marginT10 marginL20">新增演员</a>
							    	</form>
								</div> 
									...
									<table data-toggle="table" data-toolbar="#custom-toolbar1" data-pagination="true" data-page-size="15">
										<thead>
											<tr>
										        ...
										        <th data-align="center" data-width="30%">演员名</th>
										...
										<tbody class="tbody-div">
											...
											<c:forEach items="${list}" var="record" varStatus="sta">
												<tr>
													<td>${sta.index+1 }</td>
													<td>
														<a title="点击修改" href="${ctx}/movieAct/input?id=${record.id}">${record.name }</a>
											 		</td>
													<td>
														<fmt:formatDate pattern="yyyy-MM-dd HH:mm:ss" value="${record.create_time }"/>
													</td>
													<td>
														<a href="${ctx}/movieAct/input?id=${record.id}">修改</a>
			...
			$(function(){
				$('.del-btn').click(function(){
					if(window.confirm('确认删除吗？')){
						...
						$.ajax({
							type : 'post',
							url : '${ctx}/movieAct/delete?id='+id,
							success : function(result){
								if(result){
									util.success('删除成功！');
									window.location.href = '${ctx}/movieAct/list';
								}else{
									util.error('删除失败！');
								}
							}
