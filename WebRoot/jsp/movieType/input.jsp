				...
				<div class="container-fluid">
					<div class="row">
						...
						<div class="middle_top_nav">
							<a href="${ctx}/index">电影</a><a href="${ctx}/movieType/list">类型列表</a><a href="javascript:;" class="active"><c:if test="${opera eq 'create'}">新增</c:if><c:if test="${opera ne 'create'}">修改</c:if>类型</a>
						</div>
						<!--/middle_top_nav-->
						<!--/middle_main-->
						<div class="middle_main from_style">
						 <!-- list_table -->
						 <form autocomplete="off" action="${ctx}/movieType/save" method="post" class="form-horizontal">
										...
										<div class="field_box input_width">
											<div class="row">
												<div class="col-lg-6 col-md-6 marginB10">
													<label>类型名：</label>
													<input style="width:160px;" placeholder="输入类型名" name="name" class="form-control required" type="text" value="${record.name}" >
												</div>
											</div> 
									  	</div>
									</div>
